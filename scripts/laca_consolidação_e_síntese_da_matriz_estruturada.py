# -*- coding: utf-8 -*-
"""LACA Consolidação e Síntese da matriz estruturada.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1kOiwv4OCUzhVSq_hSf9S-dwbGiwHzTa3
...
### 🎯 Resultados Finais da Dissertação e Instruções para Reprodução Integral

Embora a exibição gráfica deste notebook foque na contagem bruta, o *dataframe* `df_consolidated` resultante do código acima já contém todos os dados estruturados e os identificadores `cluster_id` necessários para obter os resultados finais do PTT.

**1. Consolidação dos 909 Instrumentos Únicos**
A aplicação do agrupamento hierárquico reduz a matriz de 1.025 registros para **909 instrumentos únicos**. Para gerar a tabela e gráfico deduplicados via código, utilize o comando:
```python
# Comando para filtrar 909 instrumentos únicos
df_deduplicado = df_consolidated.drop_duplicates(subset=['cluster_id'])
df_deduplicado.groupby('categ')['nm_ins'].count().sort_values(ascending=False)
...
df_agentico = df_consolidated[
    df_consolidated['uso_evid_orq_mult_IA'].notna() &
    (~df_consolidated['uso_evid_orq_mult_IA'].astype(str).str.lower().str.contains('nenhum'))
]
...
3. Exportação do Arquivo Final do PTT
Para salvar a base final consolidada com os clusters atribuídos diretamente em arquivo Excel:

# Comando para exportar a planilha final
df_consolidated.to_excel('instrumentos_deduplicados_final.xlsx', index=False)

### 🏁 Guia de Reprodução Final: Deduplicação (909) e Base de Governo Agêntico (176)

Com a atribuição do identificador `cluster_id` executada na etapa anterior, a geração das planilhas finais do PTT é realizada via pós-processamento simples no *DataFrame* em memória. Para fins de replicação por terceiros, seguem as sintaxes em Python recomendadas:

1. **Geração da Base de 909 Instrumentos Únicos:**
   ```python
   # Seleciona a primeira ocorrência de cada cluster semântico
   df_909_unicos = df_consolidated.drop_duplicates(subset=['cluster_id']).copy()
   df_909_unicos.to_excel('/content/base_909_instrumentos_unicos.xlsx', index=False)
   print(f"Total de instrumentos únicos exportados: {len(df_909_unicos)}")

# Filtra os registros que possuem evidências atreladas ao escopo agêntico/IA
df_176_agentico = df_consolidated[
    df_consolidated['uso_evid_orq_mult_IA'].notna() &
    (df_consolidated['uso_evid_orq_mult_IA'] != '')
].copy()
df_176_agentico.to_excel('/content/base_176_governo_agentico.xlsx', index=False)
print(f"Total de registros de Governo Agêntico exportados: {len(df_176_agentico)}")
"""
